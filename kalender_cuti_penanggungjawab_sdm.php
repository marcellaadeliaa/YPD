...
$semua_data_pengajuan = [
    [ 'nama_karyawan' => 'Gilang', 'divisi' => 'SDM', 'tanggal_pengajuan' => '2025-10-25', 'jenis' => 'Cuti', 'status' => 'Menunggu' ],
    [ 'nama_karyawan' => 'Ivan', 'divisi' => 'SDM', 'tanggal_pengajuan' => '2025-10-15', 'jenis' => 'Cuti', 'status' => 'Menunggu' ],
    [ 'nama_karyawan' => 'Hana', 'divisi' => 'SDM', 'tanggal_pengajuan' => '2025-09-20', 'jenis' => 'Cuti', 'status' => 'Diterima' ],
];
// ... (Logika Kalender) ...
$data_cuti_divisi = [];
foreach ($semua_data_pengajuan as $p) { if ($p['jenis']==='Cuti' && $p['divisi']===$divisi_pj) { $data_cuti_divisi[]=$p; } }
$month = $_GET['month'] ?? 10; $year = $_GET['year'] ?? 2025;
...
$cuti_by_date=[];foreach($data_cuti_divisi as $c){$d=$c['tanggal_pengajuan'];if(!isset($cuti_by_date[$d])){$cuti_by_date[$d]=[];}$cuti_by_date[$d][]=$c;}
...
<html lang="id">
<head>
    <title>Kalender Cuti Divisi - SDM</title>
    <style>
...
<main>
    <div class="card">
        ...
        <div class="calendar">
            ...
            <?php for($day=1;$day<=$days_in_month;$day++): $current_date=date('Y-m-d',mktime(0,0,0,$month,$day,$year));$is_today=$current_date==date('Y-m-d');$has_cuti=isset($cuti_by_date[$current_date]);?>
            <div class="calendar-day <?=$is_today?'today':''?>" onclick="showDetails('<?=$current_date?>')">
                <div class="day-number"><?=$day?></div>
                <?php if($has_cuti): foreach($cuti_by_date[$current_date] as $item): $status_class='status-'.strtolower($item['status']);?>
                <span class="cuti-indicator <?=$status_class?>"><?=htmlspecialchars($item['nama_karyawan'])?></span>
