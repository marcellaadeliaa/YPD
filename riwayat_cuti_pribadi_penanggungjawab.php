...
// Kode ini menggantikan data dummy dengan query ke database.
// Anda perlu menyesuaikannya dengan skema database dan koneksi Anda.
...
// 1. Buat koneksi ke database (sesuaikan dengan kredensial Anda)
$conn = new mysqli("localhost", "username", "password", "nama_database");
if ($conn->connect_error) {
    die("Koneksi ke database gagal: " . $conn->connect_error);
}

// 2. Buat query dasar untuk mengambil riwayat cuti pribadi penanggung jawab
$sql = "SELECT id, tanggal_cuti, jenis_cuti, status, waktu_persetujuan 
        FROM pengajuan_cuti 
        WHERE kode_karyawan = ?";

$params = [$user_data['nik']];
$types = "s"; // 's' untuk string NIK

// 3. Tambahkan filter ke query jika ada
if (!empty($start_date)) {
    $sql .= " AND tanggal_cuti >= ?";
    $params[] = $start_date;
    $types .= "s";
}
if (!empty($end_date)) {
...
if (!empty($search_query)) {
    $sql .= " AND (LOWER(jenis_cuti) LIKE ? OR LOWER(status) LIKE ?)";
...
$sql .= " ORDER BY tanggal_cuti DESC";

// 4. Eksekusi query dengan prepared statement untuk keamanan
...
<style>
    ...
    @media (max-width: 768px) { .filter-row { flex-direction: column; } .filter-group { width: 100%; } .action-bar { flex-direction: column; } .btn { width: 100%; } }
</style>
</head>
<body>
...
<main>
    ...
    <p class="admin-title">Riwayat Pengajuan Cuti Pribadi Anda</p>
    ...
    <div class="card">
        ...
        <div class="filter-section">
            <form method="GET" action="riwayat_cuti_pribadi_penanggungjawab.php">
                ...
                <div class="action-bar">
                    ...
                    <a href="riwayat_cuti_pribadi_penanggungjawab.php" class="btn btn-reset">Reset Filter</a>
                    <a href="pengajuancuti_penanggungjawab.php" class="btn btn-ajukan">+ Ajukan Cuti Baru</a>
                </div>
            </form>
        </div>
