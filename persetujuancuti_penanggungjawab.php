...
if (isset($_GET['action']) && isset($_GET['id'])) {
    $action = $_GET['action'];
    $id = (int)$_GET['id'];
    $new_status = ($action == 'approve') ? 'Diterima' : 'Ditolak';
    
    $stmt = $conn->prepare("UPDATE pengajuan_cuti SET status = ?, waktu_persetujuan = NOW() WHERE id = ? AND divisi = ?");
    $stmt->bind_param("sis", $new_status, $id, $divisi_pj);
    if ($stmt->execute()) { $pesan_aksi = "Pengajuan Cuti ID #$id telah di-".$new_status; }
    $stmt->close();
}

$pengajuan_cuti = [];
$sql = "SELECT id, kode_karyawan, nama_karyawan, jenis_cuti, tgl_mulai, tgl_selesai, keterangan, status FROM pengajuan_cuti WHERE divisi = ? AND status = 'Menunggu'";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $divisi_pj);
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) { $pengajuan_cuti[] = $row; }
$stmt->close();
$conn->close();
?>
<!DOCTYPE html>
...
<main>
    <div class="card">
        ...
        <?php if ($pesan_aksi): ?><div class="action-message"><?= htmlspecialchars($pesan_aksi) ?></div><?php endif; ?>
        <div class="table-responsive">
            <table class="data-table">
                <thead>
                    <tr><th>ID</th><th>Kode</th><th>Nama</th><th>Jenis Cuti</th><th>Mulai</th><th>Selesai</th><th>Keterangan</th><th>Status</th><th>Aksi</th></tr>
                </thead>
                <tbody>
                    <?php if (!empty($pengajuan_cuti)): ?>
                        ...
                        <tr>
                            ...
                            <td><span class="status"><?= htmlspecialchars($cuti['status']) ?></span></td>
                            <td class="action-links">
                                <a href="?action=approve&id=<?= $cuti['id'] ?>" class="action-approve" title="Setujui" onclick="return confirm('Anda yakin?')">✓</a>
                                <a href="?action=reject&id=<?= $cuti['id'] ?>" class="action-reject" title="Tolak" onclick="return confirm('Anda yakin?')">✗</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="9" class="no-data">Tidak ada pengajuan cuti yang menunggu persetujuan.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
