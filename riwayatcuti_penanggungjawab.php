<?php
// FILE: riwayatcuti_penanggungjawab.php
$divisi_pj = "IT";
$riwayat_cuti = [
    ['nama_karyawan' => 'Andi', 'tgl_mulai' => '2025-10-05', 'status' => 'Menunggu Persetujuan'],
    ['nama_karyawan' => 'Eko', 'tgl_mulai' => '2025-09-20', 'status' => 'Disetujui'],
    ['nama_karyawan' => 'Fani', 'tgl_mulai' => '2025-09-15', 'status' => 'Ditolak'],
];
...
<head>
    ...
    <title>Riwayat Cuti Karyawan</title>
    <style>
...
<main>
    <div class="card">
        <h2>Riwayat Pengajuan Cuti (Divisi <?= $divisi_pj ?>)</h2>
        <table class="data-table">
            <thead><tr><th>Nama Karyawan</th><th>Tanggal Mulai</th><th>Status</th></tr></thead>
            <tbody>
                <?php if (!empty($riwayat_cuti)): foreach($riwayat_cuti as $cuti): ?>
                    <tr>
                        <td><?= htmlspecialchars($cuti['nama_karyawan']) ?></td>
                        <td><?= date('d-m-Y', strtotime($cuti['tgl_mulai'])) ?></td>
                        <td><?= getStatusBadge($cuti['status']) ?></td>
                    </tr>
                <?php endforeach; else: ?>
                    <tr><td colspan="3" style="text-align:center;">Belum ada riwayat cuti.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
