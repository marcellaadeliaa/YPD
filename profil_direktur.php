...
// Cek apakah pengguna sudah login dan memiliki peran 'direktur'
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'direktur') {
    header("Location: login_karyawan.php?error=unauthorized");
    exit();
}

// --- 1. PANGGIL KONEKSI DARI CONFIG.PHP ---
require_once 'config.php'; // Memanggil file koneksi database terpusat

// --- 2. AMBIL ID DARI SESSION ---
// Ambil ID karyawan dari session yang sedang login
$id_karyawan = $_SESSION['user']['id_karyawan']; 

$data = null;
// Query untuk mengambil data profil berdasarkan ID dari session
...
// Koneksi ditutup (opsional, bisa juga ditutup di config.php jika ada logic khusus)
$conn->close();
?>
<!DOCTYPE html>
<html lang="id">
