...
$semua_data_pengajuan = [
    [ 'nama_karyawan' => 'Putra', 'divisi' => 'Keuangan', 'tanggal_pengajuan' => '2025-09-27', 'jenis' => 'KHL', 'status' => 'Diterima' ],
];
// ... (Logika Kalender) ...
$data_khl_divisi = [];
foreach ($semua_data_pengajuan as $p) { if ($p['jenis']==='KHL' && $p['divisi']===$divisi_pj) { $data_khl_divisi[]=$p; } }
$month = isset($_GET['month']) ? (int)$_GET['month'] : 9;
$year = isset($_GET['year']) ? (int)$_GET['year'] : 2025;
// ... (sisa logika kalender sama) ...
...
$first_day=mktime(0,0,0,$month,1,$year);$days_in_month=date('t',$first_day);
$first_day_of_week=date('w',$first_day);$first_day_of_week=$first_day_of_week==0?6:$first_day_of_week-1;
$khl_by_date=[];foreach($data_khl_divisi as $k){$d=$k['tanggal_pengajuan'];if(!isset($khl_by_date[$d])){$khl_by_date[$d]=[];}$khl_by_date[$d][]=$k;}
...
<main>
    <div class="card">
        ...
        <div class="calendar">
            ...
            <?php for($day=1;$day<=$days_in_month;$day++): $current_date=date('Y-m-d',mktime(0,0,0,$month,$day,$year));$is_today=$current_date==date('Y-m-d');$has_khl=isset($khl_by_date[$current_date]);?>
            <div class="calendar-day <?=$is_today?'today':''?>" onclick="showDetails('<?=$current_date?>')">
                <div class="day-number"><?=$day?></div>
                <?php if($has_khl): foreach($khl_by_date[$current_date] as $item): $status_class='status-'.strtolower($item['status']);?>
                <span class="khl-indicator <?=$status_class?>"><?=htmlspecialchars($item['nama_karyawan'])?></span>
